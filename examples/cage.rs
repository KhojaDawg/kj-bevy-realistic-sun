...
fn main() {
    ...
    {
        App::new()
            .add_plugins((DefaultPlugins, RealisticSunDirectionPlugin, example::ExamplePlugin))

            .run();
    }
    #[cfg(not(feature="dev_features"))]
    println!("Must enable `dev_features` feature flag to tun this example");
}
...
mod example {
    ...
    impl Plugin for ExamplePlugin {
        fn build(&self, app: &mut App) {
            app.add_systems(Startup, (spawn_entities, spawn_ui));
