...
use bevy::{
    prelude::*, camera::{Exposure, Hdr}, core_pipeline::tonemapping::Tonemapping,
...
fn main() {
    ...
    {
        App::new()
            .add_plugins((DefaultPlugins, RealisticSunDirectionPlugin, dev_features::ExamplePlugin))
            .run();
    }
    #[cfg(not(feature="dev_features"))]
    println!("Enable `dev_features` feature to run this example");
}
...
mod dev_features {
    ...
    impl Plugin for ExamplePlugin {
        fn build(&self, app: &mut App) {
            app.add_systems(Startup, (
                spawn_camera, spawn_floor, spawn_objects, spawn_atmosphere, spawn_sun, spawn_ui,
            ));
    ...
    const UI_BACKGROUND: Color = Color::srgba(0.0, 0.0, 0.0, 0.8);
    ...
    fn draw_gizmos(mut gizmos: Gizmos){ gizmos.axes(Transform::default(), OBJECT_SIZE); }
        ...
        for (mut text, label) in &mut labels {
            text.0 = match label {
                EnvironmentOutputLabel::TimeOfDay => {
                    let hours = environment.time_of_day * RAD_TO_HOURS + 12.0;
                ...
                EnvironmentOutputLabel::Latitude => format!(
                    ...
                    environment.latitude * RAD_TO_DEG,
                ),
                EnvironmentOutputLabel::AxialTilt => format!(
                    "{:.3} rad ({:.1} deg)",
                    environment.axial_tilt,
                    environment.axial_tilt * RAD_TO_DEG,
                ),
            };
        }
    }
    ...
    fn spawn_atmosphere(
        mut commands: Commands, mut scattering_mediums: ResMut<Assets<ScatteringMedium>>,
    ) {
    ...
    fn spawn_ui(mut commands: Commands){
        ...
        let background_color = BackgroundColor(UI_BACKGROUND);
        // left pane - camera
        commands.spawn((
            Node{
